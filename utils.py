...
def greedy_action(ddqn:DDQN, state:torch.Tensor)->int:
    """Select action according to a given DQN
    ...
    Returns:
        Greedy action according to DDQN
    """
    return int(torch.argmax(ddqn(state)))

def epsilon_greedy(epsilon:float, ddqn:DDQN, state:torch.Tensor)->int:
    """Sample an epsilon-greedy action according to a given DQN
    
    Args:
        epsilon: parameter for epsilon-greedy action selection
        dqn: the DDQN that selects the action
        state: state at which the action is chosen
...
def update_target(target_ddqn:DDQN, policy_ddqn:DDQN):
    """Update target network parameters using policy network.
    Does not return anything but modifies the target network passed as parameter
    
    Args:
        target_dqn: target network to be modified in-place
        policy_dqn: the DDQN that selects the action
    """

    target_ddqn.load_state_dict(policy_ddqn.state_dict())

def loss(policy_ddqn:DDQN, target_ddqn:DDQN,
         states:torch.Tensor, actions:torch.Tensor,
         rewards:torch.Tensor, next_states:torch.Tensor, dones:torch.Tensor)->torch.Tensor:
    """Calculate Bellman error loss
    
    Args:
        policy_dqn: policy DDQN
        target_dqn: target DDQN
        states: batched state tensor
        actions: batched action tensor
        rewards: batched rewards tensor
        next_states: batched next states tensor
    ...
    Returns:
        Float scalar tensor with loss value
    """
    # Use the policy network instead of target network to select bellman target action 
    policy_ddqn_actions = policy_ddqn(next_states).max(1).indices.reshape([-1,1])
    bellman_targets = (~dones).reshape(-1)*(target_ddqn(next_states)).gather(1,policy_ddqn_actions).reshape(-1)+rewards.reshape(-1)
    q_values = policy_ddqn(states).gather(1, actions).reshape(-1)
    return ((q_values - bellman_targets)**2).mean()